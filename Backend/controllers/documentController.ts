...
export const getAllDocuments = async (req: Request, res: Response) => {
    try {
        ...
        if (user.role === 'employee') {
            ...
            const employee = await Employee.findOne({ user_id: user.userId });
            if (!employee) {
            ...
            const userWithDept = await User.findById(user.userId).populate('department');
            if (userWithDept?.department) {
                const employeesInDept = await Employee.find({
                    department: (userWithDept.department as any)._id
                }).select('_id');
        ...
        const result: IDocument[] = documents.map(doc => ({
...
export const getDocument = async (req: Request, res: Response) => {
    try {
        ...
        if (user.role === 'employee') {
            ...
            if (
                !employee ||
                document.employee_id.toString() !== (employee._id as mongoose.Types.ObjectId).toString()
            ) {
...
export const updateDocument = async (req: Request, res: Response) => {
    try {
        ...
        // Update fields
        if (name) document.doc_type = name;
        if (doc_type) document.doc_type = doc_type;
        if (employee_id) {
            const employee = await Employee.findById(employee_id);
            if (!employee) {
                return res.status(400).json({ message: 'Employee not found' });
        ...
        const updatedDocument = await EmployeeDocument.findById(id)
            .populate('employee_id', 'employee_code')
            .populate('uploaded_by', 'firstName lastName');
...
export const updateDocumentRequest = async (req: Request, res: Response) => {
    try {
        ...
        if (hr_notes !== undefined) request.hr_notes = hr_notes;
        if (fulfilled_document_id) request.fulfilled_document_id = fulfilled_document_id;
...
export const getDocumentRequest = async (req: Request, res: Response) => {
    try {
        ...
        if (user.role === 'employee') {
            const employee = await Employee.findOne({ user_id: user.userId });
            if (!employee || request.employee_id.toString() !== (employee._id as mongoose.Types.ObjectId).toString()) {
                return res.status(403).json({ message: 'Access denied' });
            }
        }
