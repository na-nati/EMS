...
// Mock implementation for v0 preview. In your actual application,
// replace this with your real AuthContext.
...
const Attendance = () => {
          ...
          <Dialog open={showMarkAttendance} onOpenChange={setShowMarkAttendance}>
            ...
            <DialogContent className="max-w-md bg-card border-border text-foreground">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="checkIn" type="time" className="bg-background border-border text-foreground" />
                  </div>
                  <div>
                    <Label htmlFor="checkOut">Check Out</Label>
                    <Input id="checkOut" type="time" className="bg-background border-border text-foreground" />
                  </div>
                </div>
              </div>
              <DialogFooter className="flex flex-col sm:flex-row sm:justify-end gap-2 mt-6">
                ...
                <Button className="flex-1 sm:flex-none bg-primary hover:bg-primary/80">Mark Attendance</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          <Dialog open={showCorrectAttendance} onOpenChange={setShowCorrectAttendance}>
            ...
            <DialogContent className="max-w-md bg-card border-border text-foreground">
              ...
              <DialogFooter className="flex flex-col sm:flex-row sm:justify-end gap-2 mt-6">
                ...
                <Button className="flex-1 sm:flex-none bg-primary hover:bg-primary/80">Apply Correction</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
  ...
  const getTabsList = () => {
    ...
    tabs.push({ value: "records", label: "Attendance Records" })
    if (canViewAnalytics) {
    ...
    <div className="space-y-6 p-4 sm:p-6 bg-background min-h-screen text-foreground font-inter">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {getAttendanceStats().map((stat) => (
          <div key={stat.title} className="bg-card p-4 sm:p-6 rounded-xl border border-border">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        <TabsList
          className={`grid w-full ${getTabsList().length === 1 ? "grid-cols-1" : getTabsList().length === 2 ? "grid-cols-2" : getTabsList().length === 3 ? "grid-cols-3" : "grid-cols-4"} bg-muted/30`}
        >
          {getTabsList().map((tab) => (
            <TabsTrigger
              key={tab.value}
                  ...
                  {filteredData
                    ...
                    .map((record) => (
                      <tr key={record.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3 text-sm text-foreground">{record.totalHours}h</td>
                        <td className="px-4 py-3">
                          <Badge className={`${getStatusBadge(record.status)} capitalize`}>{record.status}</Badge>
                        </td>
                        {canCorrectDiscrepancies && (
                          <td className="px-4 py-3">
                            <Button
            ...
            <div className="space-y-4">
              {filteredData.map((record) => (
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center space-x-0 sm:space-x-4 gap-2 sm:gap-0">
